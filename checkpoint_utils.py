...
def get_checkpoint(Args) -> Checkpoint:
    ...
    if Args.MODEL_ID is None:
        ...
        if Args.RESET_MODEL:
            ...
            Args.MODEL_ID = create_model_id()

            generator = torch.nn.Sequential(nn.Linear(2, 2))
            g_optimizer = torch.optim.Adam(generator.parameters(), Args.LR, Args.BETAS)

            discriminator = torch.nn.Sequential(nn.Linear(2, 2))
    ...
    elif not id_is_valid(Args.MODEL_ID):
